...
;; 行列の初期化データ付き生成
(define (eigen-array ms me ns ne . inits)
  (rlet1 A (make-eigen-array ms me ns ne 0)
...
(define-method eigen-array-nearly=? ((A <f64array>)
  ...
  (let ((data1 (slot-ref A 'backing-storage))
        ...
        (m2    (array-length B 0))
        (n2    (array-length B 1)))
    (unless (and (= m1 m2) (= n1 n2))
      (error "array shape mismatch"))
    (eigen-matrix-nearly-p data1 m1 n1 data2 m2 n2 precision)))
...
(define-method eigen-array-nearly-zero? ((A <f64array>)
  ...
  (let ((data1 (slot-ref A 'backing-storage))
        (m1    (array-length A 0))
        (n1    (array-length A 1)))
    (eigen-matrix-nearly-zero-p data1 m1 n1 precision)))
...
(define-macro (define-eigen-array-op op1)
  `(define-method ,(symbol-append 'eigen-array- op1) ((A <f64array>) (B <f64array>))
     ...
     (let ((data1 (slot-ref A 'backing-storage))
       ...
       (unless (and (= m1 m2) (= n1 n2))
         (error "array shape mismatch"))
       (let* ((C     (make-eigen-array 0 m1 0 n1))
              (data3 (slot-ref C 'backing-storage)))
         (,(symbol-append 'eigen-matrix- op1) data1 m1 n1 data2 m2 n2 data3)
         C))))
;; (行列2個の演算(破壊的変更版))
(define-macro (define-eigen-array-op! op1)
...
(define-macro (define-eigen-array-op-scalar op1 op2)
  `(define-method ,(symbol-append 'eigen-array- op1) ((A <f64array>) (r <real>))
     ...
     (let ((data1 (slot-ref A 'backing-storage))
           (m1    (array-length A 0))
           (n1    (array-length A 1)))
       (let* ((B     (make-eigen-array 0 m1 0 n1))
              (data2 (slot-ref B 'backing-storage)))
         (,(symbol-append 'eigen-matrix- op2) data1 m1 n1 r data2)
         B))))
;; (行列とスカラーの演算(破壊的変更版))
(define-macro (define-eigen-array-op-scalar! op1 op2)
...
(define-macro (define-eigen-array-op-unary op1)
  `(define-method ,(symbol-append 'eigen-array- op1) ((A <f64array>))
     ...
     (let ((data1 (slot-ref A 'backing-storage))
       ...
       (let* ((B     (make-eigen-array 0 m1 0 n1))
              (data2 (slot-ref B 'backing-storage)))
         (,(symbol-append 'eigen-matrix- op1) data1 m1 n1 data2)
         B))))
;; (行列1個の演算(破壊的変更版))
(define-macro (define-eigen-array-op-unary! op1)
...
(define-method eigen-array-mul ((A <f64array>) (B <f64array>))
  ...
  (let ((data1 (slot-ref A 'backing-storage))
    ...
    (unless (= n1 m2)
      (error "array shape mismatch"))
    (let* ((C     (make-eigen-array 0 m1 0 n2)) ; 結果は m1 x n2 になる
           (data3 (slot-ref C 'backing-storage)))
      (eigen-matrix-mul data1 m1 n1 data2 m2 n2 data3)
      C)))
...
(define-method eigen-array-mul! ((C <f64array>) (A <f64array>) (B <f64array>))
  ...
  (let ((data1 (slot-ref A 'backing-storage))
        ...
        (m3    (array-length C 0))
        (n3    (array-length C 1)))
    (unless (and (= n1 m2) (= m1 m3) (= n2 n3)) ; 結果は m1 x n2 になる
      (error "array shape mismatch"))
    (eigen-matrix-mul data1 m1 n1 data2 m2 n2 data3)
    C))
...
(define-method eigen-array-min ((A <f64array>))
  ...
  (let ((data1 (slot-ref A 'backing-storage))
        ...
        (n1    (array-length A 1)))
    ;; (0を許可すると実行時エラーになる)
    (unless (and (> m1 0) (> n1 0))
      (error "invalid array shape"))
    (eigen-matrix-min data1 m1 n1)))
...
(define-method eigen-array-trace ((A <f64array>))
  ...
  (let ((data1 (slot-ref A 'backing-storage))
    ...
    (eigen-matrix-trace data1 m1 n1)))
...
(define-method eigen-array-determinant ((A <f64array>))
  ...
  (let ((data1 (slot-ref A 'backing-storage))
    ...
    (unless (= m1 n1)
      (error "array shape must be square"))
    (eigen-matrix-determinant data1 m1 n1)))

;; 単位行列を計算
(define-method eigen-array-identity ((m1 <integer>) (n1 <integer>))
  (let* ((A     (make-eigen-array 0 m1 0 n1))
         (data1 (slot-ref A 'backing-storage)))
    (eigen-matrix-identity data1 m1 n1)
    A))
...
(define-method eigen-array-transpose ((A <f64array>))
  ...
  (let ((data1 (slot-ref A 'backing-storage))
        (m1    (array-length A 0))
        (n1    (array-length A 1)))
    (let* ((B     (make-eigen-array 0 n1 0 m1)) ; 結果は n1 x m1 になる
           (data2 (slot-ref B 'backing-storage)))
      (eigen-matrix-transpose data1 m1 n1 data2)
      B)))
...
(define-method eigen-array-transpose! ((B <f64array>) (A <f64array>))
  ...
  (let ((data1 (slot-ref A 'backing-storage))
        ...
        (m2    (array-length B 0))
        (n2    (array-length B 1)))
    (unless (and (= m2 n1) (= n2 m1))           ; 結果は n1 x m1 になる
      (error "array shape mismatch"))
    (eigen-matrix-transpose data1 m1 n1 data2)
    B))
...
(define-method eigen-array-inverse ((A <f64array>))
  ...
  (let ((data1 (slot-ref A 'backing-storage))
    ...
    (let* ((B     (make-eigen-array 0 n1 0 m1)) ; 結果は n1 x m1 になる
           (data2 (slot-ref B 'backing-storage)))
      (eigen-matrix-inverse data1 m1 n1 data2)
      B)))
...
(define-method eigen-array-solve ((A <f64array>) (B <f64array>))
  ...
  (let ((data1 (slot-ref A 'backing-storage))
        ...
        (n2    (array-length B 1)))
    ;; (0を許可すると実行時エラーになる)
    (unless (and (> m1 0) (> n1 0) (> m2 0) (> n2 0))
      (error "invalid array shape"))
    ;; (正方行列でないと実行時エラーになる)
    (unless (= m1 n1)
    ...
    (unless (= m1 m2)
      (error "array shape mismatch"))
    (let* ((X     (make-eigen-array 0 n1 0 n2)) ; 結果は n1 x n2 になる
           (data3 (slot-ref X 'backing-storage)))
      (eigen-matrix-solve data1 m1 n1 data2 m2 n2 data3)
      X)))
...
(define-method eigen-array-solve! ((X <f64array>) (A <f64array>) (B <f64array>))
  ...
  (let ((data1 (slot-ref A 'backing-storage))
    ...
    (unless (and (= m3 n1) (= n3 n2))           ; 結果は n1 x n2 になる
...
(define-method eigen-array-row ((A <f64array>) (i1r <integer>))
  ...
  (let ((data1 (slot-ref A 'backing-storage))
    ...
    (unless (and (>= i1 0) (< i1 m1))
      (error "invalid index value"))
    (let* ((B     (make-eigen-array 0 1 0 n1))  ; 結果は 1 x n1 になる
           (data2 (slot-ref B 'backing-storage)))
      (eigen-matrix-row data1 m1 n1 data2 i1)
      B)))
...
(define-method eigen-array-row! ((B <f64array>) (A <f64array>) (i1r <integer>))
  ...
  (let ((data1 (slot-ref A 'backing-storage))
    ...
    (unless (and (>= i1 0) (< i1 m1))
      (error "invalid index value"))
    (unless (and (= m2 1) (= n2 n1))            ; 結果は 1 x n1 になる
      (error "array shape mismatch"))
    (eigen-matrix-row data1 m1 n1 data2 i1)
    B))
...
(define-method eigen-array-col ((A <f64array>) (j1r <integer>))
  ...
  (let ((data1 (slot-ref A 'backing-storage))
    ...
    (unless (and (>= j1 0) (< j1 n1))
      (error "invalid index value"))
    (let* ((B     (make-eigen-array 0 m1 0 1))  ; 結果は m1 x 1 になる
           (data2 (slot-ref B 'backing-storage)))
      (eigen-matrix-col data1 m1 n1 data2 j1)
      B)))
...
(define-method eigen-array-col! ((B <f64array>) (A <f64array>) (j1r <integer>))
  ...
  (let ((data1 (slot-ref A 'backing-storage))
    ...
    (unless (and (>= j1 0) (< j1 n1))
      (error "invalid index value"))
    (unless (and (= m2 m1) (= n2 1))            ; 結果は m1 x 1 になる
      (error "array shape mismatch"))
    (eigen-matrix-col data1 m1 n1 data2 j1)
    B))
...
(define-method eigen-array-block ((A <f64array>)
  ...
  (let ((data1 (slot-ref A 'backing-storage))
        ...
        (i1    (- i1r (array-start A 0)))
        (j1    (- j1r (array-start A 1))))
    (unless (and (>= m2 0) (>= n2 0))
      (error "invalid block size"))
    (unless (and (>= i1 0) (>= j1 0) (<= (+ i1 m2) m1) (<= (+ j1 n2) n1))
      (error "invalid block range"))
    (let* ((B     (make-eigen-array 0 m2 0 n2)) ; 結果は m2 x n2 になる
           (data2 (slot-ref B 'backing-storage)))
      (eigen-matrix-block data1 m1 n1 data2 m2 n2 i1 j1)
      B)))
...
(define-method eigen-array-block! ((B <f64array>)
  ...
  (let ((data1 (slot-ref A 'backing-storage))
        ...
        (j1    (- j1r (array-start A 1)))
        (data2 (slot-ref B 'backing-storage))
        (m2a   (array-length B 0))
        (n2a   (array-length B 1)))
    (unless (and (>= m2 0) (>= n2 0))
      (error "invalid block size"))
    (unless (and (>= i1 0) (>= j1 0) (<= (+ i1 m2) m1) (<= (+ j1 n2) n1))
      (error "invalid block range"))
    (unless (and (= m2a m2) (= n2a n2))         ; 結果は m2 x n2 になる
      (error "array shape mismatch"))
    (eigen-matrix-block data1 m1 n1 data2 m2 n2 i1 j1)
    B))
...
(define-method eigen-array-block-copy ((A <f64array>)
  ...
  (let ((data1 (slot-ref A 'backing-storage))
    ...
    (unless (and (>= m3 0) (>= n3 0))
      (error "invalid block size"))
    (unless (and (>= i1 0) (>= j1 0) (<= (+ i1 m3) m1) (<= (+ j1 n3) n1))
      (error "invalid block range for copy-from"))
    (unless (and (>= i2 0) (>= j2 0) (<= (+ i2 m3) m2) (<= (+ j2 n3) n2))
      (error "invalid block range for copy-to"))
    (let* ((C     (make-eigen-array 0 m2 0 n2)) ; 結果は m2 x n2 になる
           (data3 (slot-ref C 'backing-storage)))
      (eigen-matrix-block-copy data1 m1 n1 data2 m2 n2 data3 m3 n3 i1 j1 i2 j2)
      C)))
...
(define-method eigen-array-block-copy! ((C <f64array>)
  ...
  (let ((data1 (slot-ref A 'backing-storage))
        ...
        (j2    (- j2r (array-start B 1)))
        (data3 (slot-ref C 'backing-storage))
        (m3a   (array-length C 0))
        (n3a   (array-length C 1)))
    (unless (and (>= m3 0) (>= n3 0))
      (error "invalid block size"))
    (unless (and (>= i1 0) (>= j1 0) (<= (+ i1 m3) m1) (<= (+ j1 n3) n1))
      (error "invalid block range for copy-from"))
    (unless (and (>= i2 0) (>= j2 0) (<= (+ i2 m3) m2) (<= (+ j2 n3) n2))
      (error "invalid block range for copy-to"))
    (unless (and (= m3a m2) (= n3a n2))         ; 結果は m2 x n2 になる
      (error "array shape mismatch"))
    (eigen-matrix-block-copy data1 m1 n1 data2 m2 n2 data3 m3 n3 i1 j1 i2 j2)
    C))
