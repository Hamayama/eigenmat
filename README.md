...
2. より汎用的な行列演算モジュールである f2arrmat ( https://github.com/Hamayama/f2arrmat ) は、  
...
- 2019-5-2   v1.39 using namespace を使わないようにした  
  cmath のインクルードを追加
- 2020-1-5   v1.40 Gauche v0.9.9 対応  
  (Gaucheのヘッダーファイル内の`complex`がC++のクラスとバッティングするため、  
  Gaucheのヘッダーファイルをローカルにコピーして`_Complex`に置換するシェルスクリプトを追加  
  (Gaucheの開発最新版では対策済み))
- 2020-1-7   v1.40 シェルスクリプト見直し等
